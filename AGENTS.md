...
- Give every executable tier a unique, descriptive `id`: use `short <=272K` / `long >272K` for the current GPT policies and `short <200K` / `long >=200K` for Grok 4.3/4.6. IDs are displayed verbatim and group statistics per actual model; numeric bounds, not ID text, select rates. Preserve IDs for price-only edits; update the boundary text when changing a labeled threshold. Reusing an ID merges its historical statistics without repricing them; never rename old snapshots from the current card.
