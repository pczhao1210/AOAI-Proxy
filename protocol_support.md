...
- 多 choice 和供应商自定义 finish reason；
...
- custom tool call；
- MCP、computer、local shell 等专用 item；
- web search item；
- reasoning item；
- compaction item；
...
这些高级能力只有在上游原生支持且字段策略放行时才成立。尤其是 Azure 上游的内置工具集合可能比 OpenAI 公共接口更窄。
...
- `document`；
- signed thinking 或 redacted thinking；
...
| MCP | 无等价 | 支持 | 无完整等价 | 跨协议拒绝 |
| computer use | 无等价 | 支持 | provider 专用 | 跨协议拒绝 |
| shell/local shell | 无等价 | 支持 | 无等价 | 跨协议拒绝 |
...
只有存在有效工具时才保留 `tool_choice`、`parallel_tool_calls` 等控制项。不同协议的 tool choice 并非全等：
...
- 最后一个事件没有尾随换行；
- 原生流保留完整原始 frame，包括未解释的 `event`、`id`、`retry` 等字段；
- 跨协议流只拼接并解释 `data:` 行，事件类型依赖 JSON payload 内的 `type`，不读取 SSE 的 `event`、`id` 或 `retry` 语义；
...
| 源协议 | 合法成功或完整终态 |
| --- | --- |
| Chat | `[DONE]`，或至少一个 choice 出现非空最终 `finish_reason`；当前任一证据都可判定完整 |
| Responses | `response.completed` 或 `response.incomplete` |
| Messages | `message_stop` |

Responses 的 `response.failed` 和 provider error 是失败终态。Messages 收到 `[DONE]` 不能当作合法 `message_stop`。

终端标记之后的额外事件应忽略，避免重复 usage、重复 tool call 或二次完成。

### 14.3 原生流

协议相同时尽量保留原始 SSE frame，同时旁路观察：
...
- 模型目录通过 Claude Code User-Agent 或 `format=claude-code` 协商；
